...
function buyTicket(mailText: string) {
  ...
  tickets.forEach((ticket, index) => {
    console.log(`车票: ${ticket.index}`);
    console.log(`订单:${ticket.orderId}`);
...
async function updateDatabase(
  ...
  const insertStmt = await env.DB.prepare(`
		INSERT INTO EventsOf12306 (orderNumber, sequence, buyer, departureTime, startStation, endStation, trainNumber, seatNumber, gate)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
	`);
